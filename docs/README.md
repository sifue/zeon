...
- ZEN大学のGoogleアカウント(zen.ac.jpドメインおよびstudent.zen.ac.jp)でのサインアップができる

### BANされたユーザー
- ログインすると、直後にログアウトし、BANされていることを通知する
- zen.ac.jpドメインおよびstudent.zen.ac.jp以外のGoogleアカウントでログインした場合、BANされていることを通知する
...
- 特定のレビューに対するユニークURLを発行することができる。URLの形式は `/subject/[科目コード]?evaluation=[評価ID]` となる。
- ユニークURLにアクセスすると、該当するレビューが強調表示され、自動的にスクロールされる。
